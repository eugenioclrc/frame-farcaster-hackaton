export default class Svg {
    constructor(opts = {width: 60, height: 40, fontSize: 16, fontFamily: 'monospace'}) {
        this.matrix = [];
        this.opts = opts;
        this.messages = [];
    }
            ...
            for(let x = 0; x < this.opts.width; x++) {
                ...
                if(cell){
                    if (this.matrix[y][x+1] && this.matrix[y][x+1].bg == cell.bg) {
                        let endX;
                        for(endX = x; x < this.opts.width; endX++) {
                            if (!this.matrix[y][endX] || this.matrix[y][x].bg != cell.bg) {
                                break;
                            } else {
                                renderSquare[`x${endX}y${y}`] = true;
                            }
                        }
                        //svg += `<rect x="${x*this.opts.fontSize + 4}" y="${y*this.opts.fontSize+6}" width="${(endX - x)*this.opts.fontSize + 3 }" height="${this.opts.fontSize+12}" fill="${cell.bg}" />`;
