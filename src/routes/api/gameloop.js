...
export default async function render(userId, action) {
    ...
    */
    ...
    // const instructions = document.getElementById('game-instructions');
        ...
        if (currentKeyHandler() === handlePlayerKeys) {
            ...
            let hasSavedGame = false;//window.localStorage.getItem(STORAGE_KEY) !== null;
    ...
    */
    ...
    //loadState();
    await runAction(['restore-game']);

    console.log("action:", action)
    if(action == 1) {
        //handlePlayerKeys('ArrowUp');
        await runAction(['move', 0, -1])
        console.log('ArrowUp')
    } else if(action == 2) {
        //handlePlayerKeys('ArrowDown');
        await runAction(['move', 0, +1])
        console.log('ArrowDown')
    } else if(action == 3) {
        //handlePlayerKeys('ArrowLeft');
        await runAction(['move', -1, 0])
        console.log('ArrowLeft')
    } else if(action == 4) {
        //handlePlayerKeys('ArrowRight');
        await runAction(['move', +1, 0])
